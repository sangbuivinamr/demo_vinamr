...
//Default url
const URL_QUOTA_INFORMATION =
  "https://115.73.222.254:8000/quota/getQuotaInformation/";
const URL_EXPRESSION =
  "https://115.73.222.254:8000/expression/expressionReview/";
const URL_POST_QUOTA_INFORMATION =
  "https://115.73.222.254:8000/quota/quotaInformation";
...
const QuotaManagement = (props) => {
  ...
  const onSwappingQuotaRow = (swapType) => {
    ...
    for (quotaIndex; quotaIndex <= currentQuotaData.length - 1; quotaIndex++) {
      if (
        currentQuotaData[quotaIndex]["name"] === quotaClickStatus.quotaLabel
      ) {
    ...
    if (swapType === "UP") {
      if (selectedQuotaIndex > 0) {
        currentQuotaData[selectedQuotaIndex] =
          currentQuotaData[selectedQuotaIndex - 1];
  ...
  const onAddingQuota = (projectId) => {
    ...
    if (quotaInput.name !== "" && quotaInput.expression !== "") {
      axios
        .post(
          URL_POST_QUOTA_INFORMATION + `?projectId=${projectId}`,
          newQuotaData
        )
        .then((res) => {
          console.log("res", res);
  ...
  const onDeletingQuota = () => {
    ...
    let newQuotaData = currentQuotaData.filter(
      (quota) => quota.name !== quotaClickStatus.quotaLabel
    );
    setQuotaData(newQuotaData);
  };
  ...
  const getDataInformation = async (projectId) => {
    const response = await axios.get(
      URL_QUOTA_INFORMATION + `?projectId=${projectId}`
    );
  ...
  const getDataExpression = async (projectId, code) => {
    const response = await axios.get(
      URL_EXPRESSION + `?projectId=${projectId}&code=${code}`
    );
  ...
  return (
    <div className="quota-page">
      <div className="quota-page default-bar">
        ...
        <div className="up">
          <i>
            <IoMdClose className="up icon" onClick={() => onDeletingQuota()} />
          </i>
        </div>
        <div className="up">
          <i>
            <IoIosSave className="up icon" onClick={() => onAddingQuota(1)} />
          </i>
        </div>
        ...
        <div className="expression-review">
          <h2 className="review">EXPRESSION REVIEW</h2>
          <input
